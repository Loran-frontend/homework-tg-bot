...
const webhookUrl = process.env.RENDER_EXTERNAL_URL ? `${process.env.RENDER_EXTERNAL_URL}/telegram/webhook` : undefined;
const webhookSecret = process.env.WEBHOOK_SECRET;
if (isRender && !webhookUrl) throw new Error("RENDER_EXTERNAL_URL is not set");
if (isRender && !webhookSecret) throw new Error("WEBHOOK_SECRET is not set");
...
const server = createServer(async (req, res) => {
  ...
  if (req.method === "POST" && url.pathname === "/telegram/webhook" && webhookHandler) {
    ...
    catch (error) { console.error("Telegram webhook error:", error); if (!res.headersSent) { res.writeHead(500); res.end("Internal Server Error"); } }
    return;
  }
  res.writeHead(404);
  res.end("Not Found");
