...
export class HomeworkStore {
  ...
  async getTopic(chatId: number, threadId: number, type: HomeworkType = "IRNITU", archive = false): Promise<TopicHomework> {
    const now = new Date();
    const topic = await this.prisma.topic.findFirst({ where: { group: { chatId: BigInt(chatId) }, messageThreadId: threadId }, include: { group: true, homeworkLists: { where: { type }, include: { items: { where: archive ? { archived: true } : { archived: false, OR: [{ deadline: null }, { deadline: { gt: now } }] }, orderBy: { id: "asc" } } } } } });
  ...
  async getOutputMessages(): Promise<TopicMessages> {
    await this.archiveExpired();
    const now = new Date();
    const lists = await this.prisma.homeworkList.findMany({ include: { topic: { include: { group: true } }, items: { orderBy: { id: "asc" } } }, orderBy: { id: "asc" } });
    const active: TopicHomework[] = [];
    const archive: TopicHomework[] = [];
    for (const list of lists) {
      const activeItems = list.items.filter((item) => !item.archived && (item.deadline === null || item.deadline > now));
      const archiveItems = list.items.filter((item) => item.archived);
      if (activeItems.length > 0) active.push(this.toTopicHomework(list.topic, { ...list, items: activeItems }));
      if (archiveItems.length > 0) archive.push(this.toTopicHomework(list.topic, { ...list, items: archiveItems }));
    }
    const messages = await this.prisma.outputMessage.findMany();
    const activeMessage = messages.find((message) => message.messageType === "ACTIVE");
    const archiveMessage = messages.find((message) => message.messageType === "ARCHIVE");
    return { chatId: 0, threadId: 0, activeMessageId: activeMessage && activeMessage.messageId > 0 ? activeMessage.messageId : undefined, activeChatId: activeMessage?.destinationChatId == null ? undefined : Number(activeMessage.destinationChatId), archiveMessageId: archiveMessage && archiveMessage.messageId > 0 ? archiveMessage.messageId : undefined, archiveChatId: archiveMessage?.destinationChatId == null ? undefined : Number(archiveMessage.destinationChatId), active, archive };
  }
  ...
  async setOutputDestination(messageType: PersistentMessageType, destinationChatId: number): Promise<void> {
    await this.prisma.outputMessage.upsert({ where: { messageType }, update: { destinationChatId: BigInt(destinationChatId), messageId: 0 }, create: { messageType, destinationChatId: BigInt(destinationChatId), messageId: 0 } });
  }
  ...
  async clearLegacyPersistentMessages(): Promise<void> { await this.prisma.persistentMessage.deleteMany(); }

  async archiveExpired(now = new Date()): Promise<Array<{ chatId: number; threadId: number }>> {
    const dueItems = await this.prisma.homeworkItem.findMany({ where: { archived: false, deadline: { lte: now } }, select: { id: true, list: { select: { topic: { select: { messageThreadId: true, group: { select: { chatId: true } } } } } } } });
    if (dueItems.length === 0) return [];
    await this.prisma.homeworkItem.updateMany({ where: { id: { in: dueItems.map((item) => item.id) }, archived: false, deadline: { lte: now } }, data: { archived: true } });
    const topics = new Map<string, { chatId: number; threadId: number }>();
    for (const item of dueItems) { const topic = item.list.topic; const chatId = Number(topic.group.chatId); topics.set(`${chatId}:${topic.messageThreadId}`, { chatId, threadId: topic.messageThreadId }); }
  ...
  async ensureTopic(chatId: number, threadId: number, type: HomeworkType = "IRNITU"): Promise<TopicHomework> {
    return this.prisma.$transaction(async (tx) => {
      ...
      const topic = await tx.topic.findUniqueOrThrow({ where: { id: list.topicId }, include: { group: true, homeworkLists: { where: { type }, include: { items: { where: { archived: false }, orderBy: { id: "asc" } } } } } });
  ...
  async getPersistentMessageId(chatId: number, threadId: number, messageType: PersistentMessageType): Promise<number | null> {
    const message = await this.prisma.persistentMessage.findFirst({ where: { topic: { group: { chatId: BigInt(chatId) }, messageThreadId: threadId }, messageType }, select: { messageId: true } });
  ...
  private async lockHomeworkList(tx: Prisma.TransactionClient, topicId: number, type: HomeworkType): Promise<void> { const lockKey = `homework-list:${topicId}:${type}`; await tx.$executeRaw(Prisma.sql`SELECT pg_advisory_xact_lock(hashtextextended(${lockKey}, 0))`); }
  ...
  private toTopicHomework(topic: { messageThreadId: number; group: { chatId: bigint } }, list: { type: HomeworkType; items: Array<{ id: number; authorId: number; subject: string; description: string; subgroup: HomeworkSubgroup; deadline: Date | null; archived: boolean; completed: boolean; createdAt: Date; updatedAt: Date }> }): TopicHomework { return { chatId: Number(topic.group.chatId), threadId: topic.messageThreadId, type: list.type, items: list.items.map((item) => this.toHomeworkItem(item, list.type)) }; }
  private toHomeworkItem(item: { id: number; authorId: number; subject: string; description: string; subgroup: HomeworkSubgroup; deadline: Date | null; archived: boolean; completed: boolean; createdAt: Date; updatedAt: Date }, type: HomeworkType): HomeworkItem { return { id: item.id, type, subject: item.subject, description: item.description, subgroup: item.subgroup, deadline: item.deadline, archived: item.archived, completed: item.completed, authorId: item.authorId, createdAt: item.createdAt, updatedAt: item.updatedAt }; }
}
